...
func (c *Core) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	if node == nil {
		// 如果没有找到，这里打印日志
		ctx.SetStatus(http.StatusNotFound).Json("404 not found")
		return
	}
	...
	// 调用路由函数，如果返回err 代表存在内部错误，返回 500 状态码
	if err := ctx.Next(); err != nil {
		ctx.SetStatus(http.StatusInternalServerError).Json("500 inner error")
		return
	}
}
